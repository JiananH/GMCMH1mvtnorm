  ...
  nuTuning[i] = REAL(nuTuning_r)[i];
    ...
    *****************************************/
    ...
    for(s = 0; s < nSamples; s++){
     ...
     //      dist += (tempbeta0[dim]-beta0[dim])*(tempbeta0[dim]-beta0[dim])
     //    }
     //   }
     //   beta0[0]=tempbeta0[0];
     //   beta0[1]=tempbeta0[1];
     // }
       ...
       if (sumbeta0==0){
        ...
        for (int dim=0; dim<p; dim++){
            ...
            double *pvaluecurrent=&valuecurrent;
            double *pvaluepropose=&valuepropose;
            double value=0.0;
            double *pvalue=&value;
            int inform=0;
            int *pinform=&inform;
            ...
            // covert a covariance matrix to correlation matrix
            double *tmp_pp_cor = (double *) R_alloc(pp, sizeof(double));
            tmp_pp_cor[0]=1;
            tmp_pp_cor[1]=tmp_pp[1]/std::sqrt(tmp_pp[0]*tmp_pp[3]);
            tmp_pp_cor[2]=tmp_pp[1]/std::sqrt(tmp_pp[0]*tmp_pp[3]);
            tmp_pp_cor[3]=1;
           
            // printMtrx(tmp_pp,p,p);
            // printMtrx(tmp_pp_cor,p,p);
            // mvtnorm_C_mvtdst(pnop,pnu,beta0,beta0,intin,tmp_pp,tmp_p2,pmaxipts,pabseps,preleps,perror,pvalue,pinform,prnd);
            mvtnorm_C_mvtdst(pnop,pnu,lowercurrent,uppercurrent,intin,tmp_pp_cor,tmp_p2,pmaxipts,pabseps,preleps,perror,pvaluecurrent,pinform,prnd);
            mvtnorm_C_mvtdst(pnop,pnu,lowerpropose,upperpropose,intin,tmp_pp_cor,tmp_p2,pmaxipts,pabseps,preleps,perror,pvaluepropose,pinform,prnd);
            
            // printVec(tmp_p2,p);
            // printVec(tempbeta0,p);
            // printVec(lowerpropose,p);
            // printVec(upperpropose,p);
            // printVec(beta0,p);
            // printVec(lowercurrent,p);
            // printVec(uppercurrent,p);
            // printMtrx(tmp_pp,p,p);
            // printMtrx(tmp_pp_cor,p,p);
            // printf("%f %f \n", valuecurrent, valuepropose);
